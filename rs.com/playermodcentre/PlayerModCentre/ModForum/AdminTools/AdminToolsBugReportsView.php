...
if(isset($_POST['squash'])){
        include '../../../../connect.php';
       $n = 1;
       $stmt= $conn->prepare("UPDATE bugreports SET `status` = ? WHERE bugID =?");
       $stmt->bind_param("ii",$n,$_GET['bugID']);
       $stmt->execute();
       mysqli_close($conn);
}
?>
<?php
    session_start();
    $bugID = htmlspecialchars_decode($_GET['bugID']);
    include '../../../../connect.php';

    $stmt= $conn->prepare("SELECT * FROM bugreports WHERE bugID =?");
    $stmt->bind_param("i",$_GET['bugID']);
    $stmt->execute();
    ...
    while($row = mysqli_fetch_assoc($result)){
        ...
        $message =htmlspecialchars_decode($row['bugreport']);
...
<center><b>Bug Report - View Bug Report</b></center>
<center><a href="AdminToolsBugReports.php?page=<?php echo $current_page;?>" class="c">Back to Bug Reports</a></center>
<?php
...
<td><table width="100%">
<tbody><tr><td>
<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']."?bugID=$bugID");?>" method="POST">
