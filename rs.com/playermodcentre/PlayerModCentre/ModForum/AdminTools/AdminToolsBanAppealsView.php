...
if(isset($_POST['approve'])){
        include '../../../../connect.php';
        $s = 1;
        $stmt= $conn->prepare("UPDATE banappeals SET `status`= ? WHERE appealID =?");
        $stmt->bind_param("ii",$s,$_GET['appealID']);
        $stmt->execute();
}
...
<?php
    session_start();
    $appealID = htmlspecialchars_decode($_GET['appealID']);
    include '../../../../connect.php';

    $stmt= $conn->prepare("SELECT * FROM banappeals WHERE appealID =?");
    $stmt->bind_param("i",$_GET['appealID']);
    $stmt->execute();
    ...
    while($row = mysqli_fetch_assoc($result)){
        ...
        <tr><td></td><td align="right"><b>Status:</b></td><td></td><td>'.$status.'
        </td></tr><tr><td></td><td align="right"><b>Date:</b></td><td></td><td>'.$date.'
        </td></tr>';

        $message =htmlspecialchars_decode($row['appeal']);
        $message = '<td>'.$message.'</td>';

        
        $stmt= $conn->prepare("SELECT banID FROM bandetails WHERE userID =?");
        $stmt->bind_param("i",$row['userID']);
        $stmt->execute();
        $result3 = $stmt->get_result();
        $ban = $result3->fetch_assoc();
...
<center><a href="AdminToolsBanAppeals.php?&page=<?php echo $current_page; ?>" class="c">Back to ban appeals</a></center>
<?php
...
<td><table width="100%">
<tbody><tr><td>
<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']."?appealID=$appealID");?>" method="POST">
<?php if($status < 1):?>
<img src="../../../../messagecentre/img/delete.png" style="vertical-align: middle;" alt="Deny Appeal" border="0" height="11" width="18"></a>
<input style="background-color:#000000; color:#FFFFFF;" type="submit" name="deny" value="Deny"><br>
<img src="../../../../messagecentre/img/delete.png" style="vertical-align: middle;" alt="Approve Appeal" border="0" height="11" width="18"></a>
<input style="background-color:#000000; color:#FFFFFF;" type="submit" name="approve" value="Approve"><br><br>
<?php else:?>
<?php endif;?>
&emsp;<a href="AdminToolsBanView.php?banID=<?php echo $ban['banID'];?>" class="c">View Ban</a><br>
