...
if(isset($_POST['squash'])){
        include '../../../../connect.php';
       $n = 1;
       $stmt= $conn->prepare("UPDATE technical SET `status` = ? WHERE techID =?");
       $stmt->bind_param("ii",$n,$_GET['techID']);
       $stmt->execute();
       mysqli_close($conn);
}
?>
<?php
    session_start();
    $techID = htmlspecialchars_decode($_GET['techID']);
    include '../../../../connect.php';

    $stmt= $conn->prepare("SELECT * FROM technical WHERE techID =?");
    $stmt->bind_param("i",$_GET['techID']);
    $stmt->execute();
    ...
    while($row = mysqli_fetch_assoc($result)){
        ...
        <tr><td></td><td align="right"><b>Title:</b></td><td></td><td>'.$row['title'].'
        </td></tr><tr><td></td><td align="right"><b>Status:</b></td><td></td><td>'.$status.'
        </td></tr>
        <tr><td></td><td align="right"><b>Date:</b></td><td></td><td>'.$date.'
        </td></tr>';

        $message =htmlspecialchars_decode($row['techreport']);
...
<center><b>Technical Report - View Technical Report</b></center>
<center><a href="AdminTechnicalReports.php?page=<?php echo $current_page;?>" class="c">Back to Technical Reports</a></center>
<?php
...
<td><table width="100%">
<tbody><tr><td>
<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']."?techID=$techID");?>" method="POST">
<?php if($outcome == 0):?>
<img src="../../../../messagecentre/img/delete.png" style="vertical-align: middle;" alt="Deny Appeal" border="0" height="11" width="18"></a>
<input style="background-color:#000000; color:#FFFFFF;" type="submit" name="squash" value="Squash"><br><br>
<?php else:?>
<br><br>
<?php endif?>
</form>
